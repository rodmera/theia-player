...
| Cortes de Audio / Bluetooth | El modo `audio_exclusive=yes` causa excepciones en Bluetooth (Galaxy Buds) porque carecen de interfaz ALSA exclusiva. Para evitar micro-cortes sin usar modo exclusivo durante desconexiones Bluetooth en PipeWire, se deben configurar las propiedades de suspensión de WirePlumber a nivel de sistema operativo (`session.suspend-timeout-seconds = 0` y bloquear la reducción dinámica de Quantum bajo `/etc/pipewire/`). Evitar configurar búferes estáticos forzados en `mpv` ya que causan peleas de sincronización de reloj y distorsión de paquetes en la transmisión de códecs Bluetooth comprimidos. |
