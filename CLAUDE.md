...
theiaplayer/widgets.py     ClickList, Logo, Visualizer, NowPlaying (transport animado), QueueList (con hover tooltip), SongTooltip (Container+Static child)
...
| Tooltip de Cola con `_content` Shadow | El tooltip fue refactorizado de `Static` a `Container` con un hijo `Static` interno (`_label`). Razón: en algunas versiones de textual, `Static` usa `self._content` (atributo literal single-underscore) en lugar de `self.__content` (mangled). Subclasando `Static` y escribiendo `self._content` en el `__init__`, se creaba un collision con la slot del padre, corrompiendo el payload a la propia instancia del widget y disparando `VisualError: unable to display 'SongTooltip' type` en el primer layout pass. **Cubierto por `tests/test_queue_hover.py::test_song_tooltip_does_not_shadow_static_content_attribute` y `test_song_tooltip_visualize_accepts_label_payload`** — verificados contra la versión bugueada (fallan) y contra el fix (pasan). |
