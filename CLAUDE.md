...
theiaplayer/config.py      carga player.toml; build_bindings(); filter_songs(); _normalize_keybinds (Go→Python)
...
theiaplayer/anim.py        primitivas de animación: shimmer, smooth_bar, marquee, viz (clamp energy [0,1])
theiaplayer/widgets.py     ClickList, Logo, Visualizer, NowPlaying (transport animado), QueueList (con hover overlay), SongTooltip (Container overlay flotante — usa _hover_overlay para evitar colisión con Widget._tooltip)
...
theiaplayer/screens.py     onboarding, SearchModal, InputModal, LyricsModal, modales (Roon, Signal Path, EQ, Moods, Album Versions)
...
| BINDINGS dinámicos | Aunque `TheIAPlayerApp.BINDINGS` se intente parchear en `__init__` antes de `super().__init__()`, el framework de Textual procesa y compila los atajos de teclado de forma rígida a nivel de clase (metaclasa) en tiempo de importación del módulo. Por ende, **cualquier atajo nuevo (como `ctrl+e`, `e`, `E`, `v`, `S`, `ctrl+up`, `ctrl+down`) debe ser declarado de forma estática y nativa obligatoriamente** en el listado `BINDINGS` de la clase `TheIAPlayerApp` en `theiaplayer/app.py` para ser operativo. **Este contrato está blindado por `tests/test_bindings_complete.py`** — agregar una entrada a `config.DEFAULT_KEYBINDS` sin su correspondiente `Binding(...)` en `BINDINGS` falla en CI. |
| `_pcfg` en `__init__` | Config cargada antes de `super().__init__()` y guardada como `self._pcfg`. No recargar en `on_mount` |
| Filtros en `_show_songs` | `_apply_filters()` opera sobre la lista raw. La lista `self._songs` YA está filtrada — no filtrar de nuevo en `_song_row`. La lógica pura vive en `theiaplayer.config.filter_songs(songs, filters)` y la instancia solo delega; cualquier código nuevo que necesite filtrar canciones debe llamar a esa función, nunca reinventarla (commit `f8a9ee3` documenta la regresión que dejó Auto DJ silenciosamente sin canciones). **Cubierto por `tests/test_filter_songs.py`**. |
| Hilos en `dbus-python` | `dbus-python` requiere de forma obligatoria la llamada `dbus.mainloop.glib.threads_init()` en el hilo principal antes de instanciar `dbus.SessionBus()` en hilos secundarios de Python. Omitirlo provoca un deadlock síncrono infinito en C que congela la app en negro al iniciar. **El módulo `theiaplayer.mpris` exporta `_assert_threads_inited()`** y `_run_loop()` la invoca defensivamente, así un futuro refactor que se salte la llamada muere con `RuntimeError` en vez de congelar el arranque. **Cubierto por `tests/test_mpris.py`**. |
...
| Negociación en Ghostty | La negociación interactiva de capacidades ANSI de `textual-image` con la TTY física real puede dar timeouts. Para Ghostty, la forma óptima y 100% estable es forzar de forma nativa `NAVITUI_ART=tgp` desde `theiaplayer/terminal_probe.py` (auto-aplicado al import del paquete, sin shells intermedias de Bash). El módulo también neutraliza las blocking queries de `sixel.query_terminal_support` y `tgp.query_terminal_support` para evitar el hang de startup. |
| Carátulas en CMYK | La librería `textual-image` falla al guardar imágenes en modo CMYK como PNG (lanza `OSError: cannot write mode CMYK as PNG` en `PixelData.to_base64()`). La conversión vive en `theiaplayer.art.cmyk_to_rgb_safe(path)` (helper puro, idempotente, nunca lanza) y `api.cover_art` la invoca tanto al escribir el `.part` como al hit de caché. **Cubierto por `tests/test_art_cmyk.py`**. |
| Cortes de Audio / Bluetooth | El modo `audio_exclusive=yes` causa excepciones en Bluetooth (Galaxy Buds) porque carecen de interfaz ALSA exclusiva. Para evitar micro-cortes y el "juego silencioso" (el reproductor sigue avanzando en la UI pero se silencia) cuando otros dispositivos Bluetooth se desconectan en PipeWire, se delega la selección del driver de salida de audio (`ao`) a `theiaplayer.player.choose_audio_driver(ao, platform)`. En Linux el default es `"pipewire"` (revertido de `"pulse"` en commit `0a12bbd`); en macOS deja `None` para que mpv use CoreAudio nativo. Cambiar el default es un one-liner testeable. **Cubierto por `tests/test_player_audio_driver.py`**. Evitar configurar búferes estáticos forzados en `mpv`. **Bit-perfect no es factible por Bluetooth** (los codecs A2DP transforman los bits y el sink es software sobre BlueZ): `audio_exclusive=true` solo aplica a hardware ALSA directo (DAC USB). `player.py::resolve_audio_exclusive` neutraliza el flag cuando el driver es `pipewire`/`pulse` para evitar el "juego silencioso" (incidente 2026-08-12, DAC iFi por LDAC). |
| Controles en Letras | Debido a que `LyricsModal` toma el foco absoluto del teclado por ser un `ModalScreen`, se deben rutear de forma explícita las teclas multimedia y de navegación (`space`, `n`, `b`, `left`, `right`) en sus `BINDINGS` hacia las acciones de la App para que el usuario no pierda el control de la reproducción. |
| Carátula de Inicio Desincronizada | Al arrancar, el resaltado automático inicial del primer track de la lista sobrescribe la carátula de inicio del tema restaurado (ya que `self.player.active` es falso al estar pausado). Para evitarlo, se carga el arte en `_start()` y se restringe `_track_highlighted` a cuando el widget `#tracks-list` tiene el foco activo (`has_focus`). |
| Colisión con Widget._tooltip de Textual | ``Widget._tooltip`` es una propiedad nativa reservada en Textual para tooltips simples de texto. Si un widget asigna un objeto Widget custom a ``self._tooltip``, ``Screen._handle_tooltip_timer`` detecta que el widget tiene tooltip, extrae ``self._tooltip``, se lo pasa al widget ``Tooltip`` interno de Textual (un ``Static``) y falla con ``VisualError: unable to display 'SongTooltip' type``. Solución: nombrar el atributo de overlay custom ``self._hover_overlay`` (nunca ``self._tooltip``). **Cubierto por ``tests/test_queue_hover.py::test_queue_list_does_not_collide_with_textual_tooltip_property``**. |
| `VizModel.energy` debe estar en [0, 1] | El campo `energy` de `anim.VizModel` representa "intensidad" (1.0 = playing, 0.0 = silent). El cálculo del target `(r - floor(r)) * energy` no clampa el resultado, así que `energy > 1.0` deja que `targets` y `heights` se desbordan past 1.0. Hoy en día `NowPlaying.viz.energy` se establece a `0.0` o `1.0` siempre, pero el type hint `float` y la falta de guarda hacen que un futuro caller (p.ej. `set_energy(2.0)` para "más fuerte") rompa el visualizer silenciosamente. **Fix:** `tick()` ahora hace `e = max(0.0, min(1.0, self.energy))` antes de usarlo. **Cubierto por `tests/test_anim.py::test_viz_model_clamps_energy_to_unit_interval`**. |
| `anim.marquee` debe manejar `width <= 0` | Antes del fix, `marquee("hello", width=-1, tick=0)` producía `doubled[0:-1]` (la string casi-completa en lugar de vacío). Ahora retorna `""` para `width <= 0`. **Cubierto por `tests/test_anim.py::test_marquee_width_zero_returns_empty` y `test_marquee_width_negative_returns_empty`**. |
| `_normalize_keybinds` empty list | Un valor `[]` en el TOML Go-style (p.ej. `playback.play_pause = []`) producía un binding con `key=""` — Textual lo acepta pero la acción nunca dispara. **Fix:** `if keys:` antes de `",".join(keys)`. **Cubierto por `tests/test_config_keybinds.py::test_normalize_keybinds_empty_list_should_keep_default`**. |
| `normalize_server` no debe destruir el scheme | `normalize_server("http://")` antes del fix strippeaba el trailing slash ANTES de detectar el scheme, dejando `http:` (sin `://`), y luego el `if "://" not in url` añadía `https://` prefijando el resultado malformado `https://http:`. **Fix:** strip el trailing slash DESPUÉS del `://`. **Cubierto por `tests/test_api_client.py::test_normalize_server_no_separator_does_not_split_scheme`**. |
| `_format_song_tooltip` sanitiza newlines | Un título con `\n` embebido (válido en tracks mal taggeados) generaba phantom lines en el tooltip de 7 secciones, offset-eando todo el layout. **Fix:** helper `_safe()` colapsa newlines/tabs/whitespace múltiple a single spaces antes de renderizar. **Cubierto por `tests/test_queue_hover.py::test_format_song_tooltip_strips_newlines_from_metadata` y `test_format_song_tooltip_handles_tabs_and_multiple_whitespace`**. |
| Tests contra bugs cross-version de textual | El bug de `_content` shadow (v2.2.0/v2.2.1) **no se reprodujo en el `.venv` del proyecto** porque la versión instalada de textual usa `self.__content` (mangled a `_Static__content`), mientras que otras versiones usan `self._content` literal — así que `self._content = Text("")` en una versión crea un atributo nuevo sin chocar, y en la otra choca con el slot del padre. **Lección:** un test que pasa en el entorno del agente NO garantiza que pase en el del usuario. Para clases que interactúan con machinery interna de textual (Static, Container, etc.) se debe: (1) probar la **pipeline completa de layout** (`render()` directo NO es suficiente — el crash sólo ocurre vía `_compositor._arrange_root` → `get_content_width` → `_render`), no sólo llamadas aisladas; (2) declarar el bug en términos del **atributo** que se corrompe (no sólo del comportamiento), de modo que un test que verifique `hasattr(widget, "_content")` también funcione en versiones donde ese atributo ya existe legítimamente. La regla operativa: si el fix depende de qué versión de textual está instalada, hay que (a) documentar la dependencia explícitamente en CLAUDE.md y (b) preferir un diseño que sea inmune a la dependencia (v2.2.2 elimina el `Static` del tooltip por completo). |
| Aspect Ratio en Carátula | Antes del fix, `CoverArt > .cover-image` tenía `width: 100%; height: 100%;` forzado, lo que distorsionaba y estiraba la imagen al cambiar el ancho de la columna derecha (`#side`) con el Splitter. Con `width: auto; height: auto; max-width: 100%; max-height: 100%;`, `ImageSize` calcula automáticamente las celdas preservando la relación de aspecto nativa (1:1 en píxeles para carátulas cuadradas) y `CoverArt` la centra en el panel. **Cubierto por `tests/test_cover_art.py::test_cover_art_square_scaling_preserves_dimensions`**. |
| Dependencia `google-genai` y API Keys | La generación de trivia/reseña en Album Spotlight (`home`) y modal Roon (`C`) requiere el SDK `google-genai>=2.0` (usando modelo `gemini-3.5-flash`). Para evitar fallos si la app se lanza desde entornos sin variables exportadas (ej. launchers gráficos), `app.py` busca `GEMINI_API_KEY` en `os.environ` y como fallback lee los archivos `.env` del ecosistema (`~/.openclaw/.env`, `~/.openclaw/workspace/.env`, `~/.config/theia-player/.env`). Si la clave no está disponible, degrada a los metadatos locales de Navidrome sin bloquear la UI. |
| Teclas Multimedia e Interceptación de Navegadores (Brave/Chromium) | Chromium/Brave intercepta por defecto las teclas de hardware multimedia del sistema (`HardwareMediaKeyHandling`) y los gestores de medios en entornos Wayland (ej. Omarchy/Quickshell) conmutan el foco al reproductor de la pestaña web (YouTube, streams) desplazando al TUI. Solución arquitectónica: (1) En `~/.config/brave-flags.conf` y `chromium-flags.conf`, añadir `--disable-features=HardwareMediaKeyHandling` para evitar que el navegador secuestre eventos directos del sistema manteniendo activa su interfaz MPRIS pasiva; (2) En `~/.config/hypr/bindings.lua`, re-enlazar las teclas `XF86Audio*` usando el operador de contingencia dinámico `theia-player <cmd> || omarchy-shell media <cmd>` (`locked = true`). Esto garantiza que mientras `theia-player` esté abierto su IPC Unix responda en <5ms con prioridad absoluta ignorando al navegador, y al cerrarse degrade automáticamente al shell de medios de Omarchy para controlar la web u otras aplicaciones. |
...
| Tooltip de Cola (Queue Hover) | `widgets.py`, `app.py` | Al pasar el mouse sobre un track de la cola aparece un tooltip flotante (`SongTooltip`) con la metadata completa: título, artista, álbum, año, género, duración, formato, bitrate, track/disc, rating, plays y starred. Se actualiza en vivo al mover el cursor entre filas y se oculta al salir del panel. v2.2.2: `SongTooltip` es un `Container` que override `render()` para retornar un `Text` directo (no usa `Static` en absoluto) — ver gotcha "Tests contra bugs cross-version de textual" abajo. |
...
| Álbum del Día con Trivia de Gemini (Home) | `app.py` | La vista `home` selecciona el "Álbum del Día" con una semilla consistente de 24 horas y descarga de fondo anécdotas, datos y curiosidades reales del álbum usando Gemini (`gemini-3.5-flash`), con caché offline y renderizado responsive saltando la lectura de forma nativa en Vim. |
| Pines de Favoritos (Pinned Favorites) | `app.py`, `config.py` | `i` — Fija o desfija cualquier playlist, vista, artista o álbum al tope del sidebar bajo una nueva sección `favorites`, persistido en el estado y previniendo colisiones de IDs mediante prefijos `pin:`. |
| Modal de Playlists Multi-Seleccionable | `app.py`, `screens.py` | `p` — Rediseñado como `PlaylistPickerModal` para permitir la multi-selección interactiva de listas (`space` para conmutar `[✔]`, y `enter`/`ctrl+s` para confirmar) y guardar el track en múltiples playlists en un único lote. |
| Carpetas de Playlists Colapsables | `app.py` | Permite contraer y expandir visual e interactivamente los encabezados de carpetas `📂` / `📁` de playlists en el sidebar presionando `enter` o haciendo click. |
| Caché Local de Letras (LRC Cache) | `app.py`, `api.py` | Guarda las letras sincronizadas en el directorio de caché local con prefijo `lyrics-` para cargarlas de forma instantánea en 0ms y habilitar su visualización offline de letras. |
| Soporte Multi-Servidor (Profile Switcher) | `app.py`, `screens.py` | `ctrl+g` — Modal interactivo de cambio de servidor en caliente con reconexión de cliente Subsonic y flushing seguro de cola para evitar cruce de tracks. |
...
| Selección de Dispositivo de Audio (Device Switcher) | `app.py`, `screens.py`, `player.py` | `ctrl+d` — Modal interactivo que consulta `audio_device_list` de `mpv` en tiempo real y te permite alternar en caliente tu dispositivo de salida de audio (parlante, DAC BT, audífonos Jack); persiste la preferencia en estado. |
| Control de Audio Absoluto (Bit-Perfect) | `player.py`, `config.py`, `app.py` | `audio_exclusive = true` en `player.toml` — Le indica a `mpv` que tome el control exclusivo de hardware directo (`alsa/hw:` y `alsa/plughw:`) del DAC o puerto de audio seleccionado, saltándose el mezclador de PipeWire/PulseAudio para reproducción bit-perfect nativa (fidelidad audiófila pura). |
