...
| Carátulas en CMYK | La librería `textual-image` falla al guardar imágenes en modo CMYK como PNG (lanza `OSError: cannot write mode CMYK as PNG` en `PixelData.to_base64()`). Se debe interceptar la carga en `theiaplayer/api.py` para convertirlas a RGB y sobreescribir el archivo en disco (tanto al descargar como al recuperar del caché). |
| Cortes de Audio / Bluetooth | El modo `audio_exclusive=yes` causa excepciones en Bluetooth (Galaxy Buds) porque carecen de interfaz ALSA exclusiva. Para evitar micro-cortes y el "juego silencioso" (el reproductor sigue avanzando en la UI pero se silencia) cuando otros dispositivos Bluetooth se desconectan en PipeWire, se define por defecto el driver de salida de audio (`ao`) como `pulse` en Linux dentro de `create_player()`. El emulador `pipewire-pulse` realiza un rescate de flujo y descorche de señal mucho más robusto que la interfaz nativa `pipewire` o `alsa`, impidiendo pérdidas de audio colaterales. Evitar configurar búferes estáticos forzados en `mpv`. |
