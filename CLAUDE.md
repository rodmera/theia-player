...
theiaplayer/widgets.py     ClickList, Logo, Visualizer, NowPlaying (transport animado), QueueList (con hover tooltip), SongTooltip (Container con render() directo, sin Static — inmune al bug cross-version de `_content`)
...
| Tooltip de Cola con `_content` Shadow | El tooltip usa `Container` que override `render()` directamente para retornar un `Text`. **No usa `Static` en absoluto** — ni como clase base, ni como hijo. Razón: en algunas versiones de textual, `Static` usa `self._content` (atributo literal single-underscore) en lugar de `self.__content` (mangled). Cualquier subclase o contenedor que aloje un `Static` deja el slot `_content` vulnerable: si en algún path futuro alguien escribe `self._content = widget`, `visualize(static_widget, widget, ...)` lanza `VisualError: unable to display '<Widget>' type` durante la pipeline de layout. v2.2.1 intentó esto con `Container` + `Static` child (`_label`) y volvió a fallar en producción porque el `_label` aún era un `Static`. v2.2.2 elimina el `Static` completamente y override `render()` para retornar `_renderable` (un `Text` plano). **Cubierto por `tests/test_queue_hover.py::test_song_tooltip_does_not_shadow_static_content_attribute` y `test_song_tooltip_render_returns_valid_renderable`**. |
